...
export default function Home() {
  ...
  // 1. Get page from URL, default to 1
  ...
  // 2. Use the saved slug, fallback to "india-tour"
  ...
  const { loading, error, data } = useQuery(GET_POSTS, {
    ...
    skip: !settingsData, // Don't run until we know which category to fetch
  });
  ...
  const cleanExcerptForLatestPost = (html = "", limit = 200) => {
    ...
    // 1. Remove HTML tags and WordPress specific ellipsis
    const textOnly = html
      .replace(/<[^>]*>/g, '')
      .replace(/\[&hellip;\]/g, '')
      .trim();

    // 2. Split into words and remove empty strings
    const words = textOnly.split(/\s+/).filter(word => word.length > 0);

    // 3. Return the limited version
    if (words.length <= limit) return textOnly;
    return words.slice(0, limit).join(" ") + "...";
  };
  ...
  // 3. New Function to update URL instead of just state
  const handlePageChange = (pageNumber) => {
    // This updates the URL, causing the component to re-render with new currentPage
      ...
      <div className="px-4 md:px-10 lg:px-20 xl:px-40 flex flex-1 justify-center py-5">
        <div className="layout-content-container w-full flex flex-col max-w-[1280px] flex-1">
          ...
          : "Professional travel writer and tour guide sharing insights from the heart of India."}
                ...
                <Link
                  href={`/author/${authorData?.slug || '#'}`}
                  className="group/author-link inline-flex items-center justify-center md:justify-start gap-2 text-accent-100 font-bold text-xs uppercase tracking-widest transition-colors"
                >
                  <span>Know More</span>
                  <svg
                    className="w-4 h-4 transition-transform duration-300 group-hover/author-link:translate-x-1"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              <label className="flex flex-col gap-3 w-full max-w-[480px]">
                ...
                <div className="flex w-full flex-1 items-stretch rounded-full h-14 shadow-sm bg-secondary border border-gray-200 transition-all duration-300 focus-within:shadow-md focus-within:border-[#ffa500] focus-within:ring-1 focus-within:ring-[#ffa500] overflow-hidden">
                  ...
                  <input
                    ...
                    /* Important: Ensure focus:ring-0 and border-none stay here so they don't override the parent div styling */
          ...
          {latestPost && currentPage === 1 && !searchQuery && (
            <div className="@container mb-10">
              {/* Added 'group' to this parent div to link text and image hover together */}
                ...
                <div className="flex flex-col gap-6 flex-1 w-full">
                  <div className="flex flex-col gap-2 text-left">
                    ...
                    <h1 className="text-4xl font-black leading-tight @[480px]:text-5xl lg:text-6xl text-slate-900">
                      {/* The title link */}
                      <Link href={latestPost.uri} className="hover:text-accent-100 transition-colors">
                        {latestPost.title}
                      </Link>
                    </h1>
                    ...
                    <div className="flex flex-col gap-3">
                      <div
                        className="font-body text-slate-500 text-base leading-relaxed"
                        dangerouslySetInnerHTML={{ __html: cleanExcerptForLatestPost(latestPost.content, 60) }}
                      />

                      <a
                        href={latestPost.uri}
                        className="group/link inline-flex items-center gap-2 text-accent-100 font-bold text-sm uppercase tracking-wider transition-colors"
                      >
                ...
                <div className="w-full aspect-video rounded-xl overflow-hidden @[864px]:w-1/2 shadow-2xl relative bg-slate-200">
                  <Link href={latestPost.uri} className="block w-full h-full">
                    <Image
                      ...
                      fill
                      /* 'group-hover:scale-110' now looks at the parent container of the whole section */
          ...
          {!searchQuery && totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 mb-20">
              <button
                ...
                className="p-2 cursor-pointer rounded-full border border-gray-200 disabled:opacity-30 hover:bg-gray-50 transition-colors"
              >
              ...
              {[...Array(totalPages)].map((_, index) => (
                <button
                  ...
                  className={`w-10 cursor-pointer h-10 rounded-full text-sm font-bold transition-all ${currentPage === index + 1
                    ? "bg-accent-100 text-white shadow-md"
